...
class PagesController extends Controller
{
    ...
    {
        return view('pages/custom/product_create_form');
    }


    public function product_create_store(Request $request)
    {
        // dd($request->all());
        if ($request->hasFile('image')) {
            ...
            $product->save();
            return redirect()->route('custom/product_list')->with('modal', 'Product created successfully.');
        } else {
            return redirect()->back()->with('modal', 'Product not created.');
        }
    }
    ...
    public function product_list()
    {
        ...
        return view('pages/custom/product_list', compact('products'));
    }

    public function product_edit(int $id)
    {
        $product = Products::findOrFail($id);
        return view('pages/custom/product_edit_form', compact('product'));
    }


    public function product_update(Request $request, int $id)
    {
        $product = Products::findOrFail($id);
        if ($request->hasFile('image')) {
            // dd($request->all());
            ...
            $product->color = $request->color;
            $product->save();
            return redirect()->route('custom/product_list')->with('modal', 'Product updated successfully.');
        } else {
            $product->name = $request->name;
            $product->slug = $request->slug;
            $product->price = $request->price;
    ...
    public function product_delete(int $id)
    {
        ...
        $product->delete();
        return redirect()->back()->with('modal', 'Product deleted successfully.');
    }
